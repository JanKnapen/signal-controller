# SignalController# SignalController# SignalController# SignalController# SignalController
...
## InstallationFastAPI service for sending and receiving Signal messages via signal-cli.
...
git clone https://github.com/JanKnapen/signal-controller.git## FeaturesA FastAPI service for sending and receiving Signal messages via signal-cli REST API.
...
- Receive messages automatically via SSE stream

1. Register your Signal phone number:

```bash- SQLite database for message history## FeaturesA FastAPI-based service for sending and receiving Signal messages via signal-cli REST API.A secure service for receiving and sending Signal messages via signal-cli REST API, designed to run on a Proxmox VM.
...
2. Create `/etc/signal-controller/.env`:- Cloudflare Tunnel support
...
```## Architecture- 📤 **Send Messages** - REST API for sending Signal messages
...
4. Start services:```- 📥 **Receive Messages** - Automatic message processing via SSE stream

```bash

sudo systemctl enable --now signal-cliInternet → Cloudflare Tunnel → VM:8888 (public) → signal-cli:8080

sudo systemctl enable --now signal-controller-public

sudo systemctl enable --now signal-controller-private                              ↓- 💾 **Message Storage** - SQLite database for history and conversations## Features## 🏗️ Architecture
...
curl -X POST http://localhost:9000/send \
  ...
  -H "Content-Type: application/json" \## Installation- ☁️ **Cloudflare Ready** - Works with Cloudflare Tunnel for SSL termination
...
curl http://localhost:9000/messages -H "X-API-Key: YOUR_KEY"

```git clone https://github.com/JanKnapen/signal-controller.git## Architecture- **Send Messages**: REST API endpoint to send Signal messagesSignalController consists of two separate interfaces:



Get conversations:cd signal-controller

```bash

curl http://localhost:9000/conversations -H "X-API-Key: YOUR_KEY"sudo ./scripts/install.sh
...
- Cloudflare TunnelInternet → Cloudflare → Cloudflare Tunnel → VM:8888 (public) → signal-cli:8080



## License**1. Register Signal number:**



MIT                                          ↓- **Message Storage**: SQLite database for message history and conversations### 1. **Public Interface** (Port 8443, exposed to internet)


```bash

sudo ./scripts/register_signal.sh         Other VMs → localhost:9000 (private API)

```

```- **Dual Interface**: - **Purpose**: Receives incoming Signal messages via webhook

**2. Configure environment file `/etc/signal-controller/.env`:**



```

SIGNAL_PHONE_NUMBER=+1234567890**Two Interfaces:**  - Public (port 8888): Receives messages from signal-cli- **Security**: Rate-limited, HTTPS only, no sending capabilities

SIGNAL_API_KEY=your_secure_key_here

```- **Public (port 8888)** - Receives messages from signal-cli (exposed via Cloudflare)



Generate key: `openssl rand -hex 32`- **Private (port 9000)** - API for sending messages (localhost only)  - Private (port 9000): API for sending messages (localhost only)- **Endpoints**:
...
**4. Start services:**### 1. Install  - `GET /health` - Health check
...
sudo systemctl enable --now signal-controller-public```bash## Architecture
...
## API Examplesgit clone https://github.com/JanKnapen/signal-controller.git### 2. **Private Interface** (Port 9000, internal only)



**Send message:**cd signal-controller



```bashsudo ./scripts/install.sh```- **Purpose**: Send messages and query stored messages
...
curl -X POST http://localhost:9000/send \
  ...
  -d '{"to": "+1234567890", "message": "Hello!"}'Internet → Cloudflare → Cloudflare Tunnel → VM:8888 (public) → signal-cli:8080- **Security**: API key authentication, bound to localhost
...
curl http://localhost:9000/messages?limit=10 \```bash
...
```bash```  - `GET /messages` - Retrieve stored messages

curl http://localhost:9000/conversations \

  -H "X-API-Key: YOUR_KEY"Follow the prompts to register your phone number (requires captcha).
...
curl http://localhost:9000/stats \## Quick Setup  - `GET /stats` - Get statistics
...
## Requirements  - `GET /health` - Health check
...
- Java 21SIGNAL_PHONE_NUMBER=+1234567890### 1. Install

- Phone number for Signal

- Cloudflare TunnelSIGNAL_API_KEY=your_secure_random_key_here
...
```bashGenerate a secure key: `openssl rand -hex 32````bash

sudo systemctl status signal-cli signal-controller-public signal-controller-private

```



**View logs:**### 4. Setup Cloudflare Tunnelcd /opt```
...
sudo journalctl -u signal-controller-private -fPoint your Cloudflare Tunnel to: `http://YOUR_VM_IP:8888`git clone https://github.com/JanKnapen/signal-controller.gitSignalController/
...
## Licensesudo systemctl enable --now signal-cli



MITsudo systemctl enable --now signal-controller-public```│   ├── config.py            # Configuration management
